  ...
  const CartForm = class extends HTMLElement {
    ...
    refresh() {
      ...
      then((response) => {
        ...
        const topCheckoutBtn = document.querySelector(".top-checkout-btn");
        const cartItemCount = newContent.querySelectorAll(".cart-item").length;

        if (cartItemCount === 0) {
          topCheckoutBtn.style.display = "none";
        } else {
          topCheckoutBtn.style.display = "block";
        }
    ...
    $formContainer.filter('[data-ajax-add-to-cart="true"]:not(.feedback-go_to_cart)').find('.product-purchase-form').on('submit', function (e) {
      ...
      $.post(shopifyAjaxAddURL, $form.serialize(), function (itemData) {
        ...
        if ($form.hasClass('feedback-add_in_modal') || $form.hasClass('feedback-add_in_modal_no_checkout')) {
          ...
          if (product.selling_plan_allocation && product.selling_plan_allocation.selling_plan.name) {
            let sellingPlanText = product.selling_plan_allocation.selling_plan.name;
            if (sellingPlanText === "Delivery every 28 Days") {
              sellingPlanText = "Monthly subscription begins after the week trial, cancel anytime.";
            }

            variantHtml += `<p class="cart-product__content__meta">${sellingPlanText}</p>`;
          }
    ...
    handleScroll = function (nav, positioning, opacity) {
      ...
      if ((positioning == "sticky" || positioning == "peek") && $announcementBar.length > 0) {
        if (scrollTop <= 50) {
          ...
          // nav.bar.hideAnnouncement(true); //SV-custom
        }
      }
    };
...
$(document).ready(function(){
  $(document).on('click', '[data-pdp-section-anchor]', function(e){
    e.preventDefault();
    $("html, body").animate(
      {
        scrollTop: $(".custom-featured-product")?.offset()?.top,
      },
      800
    );
  })
